...
namespace Game.Exploration
{
    public class ExplorationData
    {
        ...
        {
            if (ship.ShipType != ShipType.Common)
                return false;
            if (ship.ShipRarity != ShipRarity.Normal && ship.ShipRarity != ShipRarity.Rare)
                return false;
        ...
        private bool IsValidForPlanet(ShipBuild build)
        {
            if (_planet.Type == PlanetType.Infected)
                return build.Ship.Features.Regeneration;
        ...
        private static bool IsSmall(ShipBuild build)
        {
            ...
            var rarity = build.Ship.ShipRarity;
            if (rarity != ShipRarity.Normal && rarity != ShipRarity.Rare)
                return false;
